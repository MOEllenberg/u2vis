...
public class PieChart2DWrapper : GeneralVisulizationWrapper
{
    ...
    {
        _visType = VisType.PieChart2D;
        MultiDimDataPresenter presenter = gameObject.GetComponent<MultiDimDataPresenter>();
        PieChart2D pieChart2D = gameObject.GetComponent<PieChart2D>();
        ...
        pieChart2D.Initialize(presenter, _axisInformation[0].AxisPrefab, _style);
        pieChart2D.ShowAxes = _axisInformation[0].ShowAxis;
        if (_visType == VisType.PieChart3D || _visType == VisType.PieChart2D)
            pieChart2D.ShowAxes = false;
        pieChart2D.Size = _visSize;
        pieChart2D.Rebuild();
