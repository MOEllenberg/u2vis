...
public class LineChart2DWrapper : GeneralVisulizationWrapper
{
    ...
    {
        _visType = VisType.LineChart2D;
        MultiDimDataPresenter presenter = gameObject.GetComponent<MultiDimDataPresenter>();
        LineChart2D lineChart2D = gameObject.GetComponent<LineChart2D>();
        ...
        lineChart2D.Initialize(presenter, _axisInformation[0].AxisPrefab, _style);
        lineChart2D.ShowAxes = _axisInformation[0].ShowAxis;
        if (_visType == VisType.PieChart3D || _visType == VisType.PieChart2D)
            lineChart2D.ShowAxes = false;
        lineChart2D.Size = _visSize;
        lineChart2D.Rebuild();
